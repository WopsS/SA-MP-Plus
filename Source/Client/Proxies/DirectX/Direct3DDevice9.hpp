...
namespace Proxies
{
	class Direct3DDevice9 : public IDirect3DDevice9
	{
	public:

		Direct3DDevice9(IDirect3DDevice9* Original);
		virtual ~Direct3DDevice9() = default;
		...
		HRESULT __stdcall DrawIndexedPrimitive(D3DPRIMITIVETYPE Type, INT BaseVertexIndex, UINT MinIndex, UINT NumVertices, UINT StartIndex, UINT PrimitiveCount);
		...
		HRESULT __stdcall GetBackBuffer(UINT iSwapChain, UINT BackBuffer, D3DBACKBUFFER_TYPE Type, IDirect3DSurface9** ppBackBuffer);
		...
		HRESULT __stdcall GetStreamSourceFreq(UINT StreamNumber, UINT* pDivider);
		...
		HRESULT __stdcall SetStreamSourceFreq(UINT StreamNumber, UINT FrequencyParameter);

		HRESULT __stdcall SetTexture(DWORD Sampler, IDirect3DBaseTexture9* pTexture);
		...
		HRESULT __stdcall UpdateSurface(IDirect3DSurface9* pSourceSurface, const RECT* pSourceRect, IDirect3DSurface9* pDestinationSurface, const POINT* pDestinationPoint);
